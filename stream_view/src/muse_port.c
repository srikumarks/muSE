...
static muse_cell port_stream_read( muse_env *env, void *self )
{
	return muse_pread( (muse_port_t)self );
}

static void port_stream_write( muse_env *env, void *self, muse_cell sexpr, muse_char end_char )
{
	muse_pwrite( (muse_port_t)self, sexpr );
	if ( end_char )
		port_putc( end_char, (muse_port_t)self );
}

static void port_stream_close( muse_env *env, void *self )
{
	port_close((muse_port_base_t*)self);
}

static muse_stream_view_t g_port_stream_view =
{
	port_stream_read,
	port_stream_write,
	port_stream_close
...
/**
 * The default port view functions that exposes
 * the "stream" view of a port via the muse_pread
 * and muse_pwrite functions.
 */
void *port_view( muse_env *env, int id )
{
	if ( id == 'strm' )
		return &g_port_stream_view;
	else
		return NULL;
}
...
{
	/* Set the default port view if no view is found. */
	if ( ((muse_functional_object_type_t*)p->base.type_info)->view == NULL )
		((muse_functional_object_type_t*)p->base.type_info)->view = port_view;
