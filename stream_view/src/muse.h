...
/**
 * A stream view 'strm' provides a way to write and read s-expressions.
 * All ports support the stream view by virtue of muse_pread and muse_pwrite functions.
 */ 
typedef struct
{
	muse_cell (*read)( muse_env *env, void *self );
	/**<
	 * Reads one s-expression from the stream and returns it.
	 */
	 
	void (*write)( muse_env *env, void *self, muse_cell sexpr, muse_char end_char );
	/**<
	 * Writes the given s-expression to the stream.
	 */
	 
	void (*close)( muse_env *env, void *self );
	/**<
	 * Closes the stream in either direction.
	 */
} muse_stream_view_t;
