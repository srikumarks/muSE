...
static muse_stream_view_t *stream_view( muse_env *env, muse_cell stream, muse_functional_object_t **fnobj )
{
	muse_functional_object_t *fobj = _fnobjdata(stream);
	(*fnobj) = fobj;
	if ( fobj && fobj->type_info->view )
	{
		return (muse_stream_view_t*)fobj->type_info->view( fobj, 'strm' );
	}
	else
		return NULL;		
}
...
{
	/* All ports support the stream view. So we only need
	examine stream objects here. */
	muse_cell strmobj = _evalnext(&args);
	muse_functional_object_t *fobj = NULL;
	muse_stream_view_t *strm = stream_view( env, strmobj, &fobj );
	if ( strm ) 
	{
		while ( args )
		{
			muse_cell sexpr = _evalnext(&args);
			strm->write( env, fobj, sexpr, args ? ' ' : '\n' );
		}
		return _t();		
	}
	else
	{	
		return fn_output( env, _cons(_qq(strmobj), args), muse_pwrite );
	}
}
 ...
 * (read [port]) 
 *
 * Reads a single sexpr from the standard input stream or the given
 * object supporting the stream view (which any port will).
 */
muse_cell fn_read( muse_env *env, void *context, muse_cell args )
{
	if ( args )
	{
		/* All ports support the stream view. So we only
		need to check for the stream view and call the read function. */
		muse_cell strmobj = _evalnext(&args);
		muse_functional_object_t *fobj = NULL;
		muse_stream_view_t *strm = stream_view( env, strmobj, &fobj );
		if ( strm ) 
		{
			muse_cell result = strm->read( env, fobj );
			return result < 0 ? MUSE_NIL : result;
		}
		muse_assert( MUSE_FALSE && "Read can only take a stream as argument.");
		return MUSE_NIL;
	}
	else
	{
...
/**
 * (close port).
 * Closes the given stream. All ports are streams unless they
 * override the default port view.
 */
muse_cell fn_close( muse_env *env, void *context, muse_cell args )
{
	muse_cell port = _evalnext(&args);
	muse_functional_object_t *fobj = NULL;
	muse_stream_view_t *strm = stream_view( env, port, &fobj );
	if ( strm )
	{
		strm->close( env, fobj );
		return _t();
