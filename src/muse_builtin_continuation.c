...
static muse_cell capture_continuation( muse_env *env, muse_cell cont )
{
	...
	else
	{
		...
		was captured. */
		env->current_process->atomicity = c->process_atomicity;
		...
		memcpy( env->current_process->bindings_stack.bottom + c->bindings_stack_from, c->bindings_stack_copy, sizeof(muse_cell) * c->bindings_stack_size );
		env->current_process->bindings_stack.top = env->current_process->bindings_stack.bottom + c->bindings_stack_from + c->bindings_stack_size;

		/* Restore the saved symbol values. */
		restore_bindings( env, c->bindings_copy, c->bindings_size );

		muse_assert( c->invoke_result >= 0 );
		muse_assert( (env->current_process->state_bits & MUSE_PROCESS_DEAD) == 0 );
