...
sha1_t sha1_update( const unsigned char *chunk, sha1_t s )
{
...
sha1_t sha1_end( const unsigned char *chunk, size_t chunk_size, size_t total_bytes, sha1_t s )
{
...
{
	hex[0] = hexdigit((word >> 28) & 15);
	hex[1] = hexdigit((word >> 24) & 15);
	hex[2] = hexdigit((word >> 20) & 15);
	hex[3] = hexdigit((word >> 16) & 15);
	hex[4] = hexdigit((word >> 12) & 15);
	hex[5] = hexdigit((word >> 8) & 15);
	hex[6] = hexdigit((word >> 4) & 15);
	hex[7] = hexdigit(word & 15);
}

muse_cell sha1_to_str( muse_env *env, sha1_t s )
{
...
sha1_t sha1_hash_bytes( const unsigned char *bytes, size_t sz )
{
	sha1_t s = sha1_init();
	size_t i = 0;
	for ( i = 0; i < sz; i += 64 )
...
sha1_t sha1_hash_port( muse_port_t p )
{
